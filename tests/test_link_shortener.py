...
def test_valid_link_shortening(in_memory_db):
    ...
    assert any(short_link.endswith(row[0]) for row in links)
    ...
    links = in_memory_db.get_links()
    assert any(short_link_2.endswith(row[0]) for row in links)
...
def test_generate_unique_link_id_returns_valid_and_unique_id(tmp_path, monkeypatch):
